...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  uint64_t dist_prev
    = ( candidate_prev > checkpoint ) ? ( candidate_prev - checkpoint ) : ( checkpoint - candidate_prev );
  uint64_t dist_next
    = ( candidate_next > checkpoint ) ? ( candidate_next - checkpoint ) : ( checkpoint - candidate_next );
